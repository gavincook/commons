...
public class ImageUtil {
     ...
     * @param width 缩放后的宽度
     */
    public final static boolean scale(String srcImageFile, String targetImageFile, int width, int height) {
        boolean result = false;
        try {
            BufferedImage src = ImageIO.read(getImageFile(srcImageFile)); // 读入文件
            Image image = src.getScaledInstance(width, height, Image.SCALE_DEFAULT);
            BufferedImage tag = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
            Graphics g = tag.getGraphics();
            g.drawImage(image, 0, 0, null); // 绘制缩小后的图
            g.dispose();
            ImageIO.write(tag, ImageType.JPG.getName(), new File(targetImageFile));// 输出到文件流
            result = true;
        } catch (IOException e) {
            e.printStackTrace();
            logger.error("ImageUtil scale from {} to {} failed!", srcImageFile, targetImageFile);
        }
        return result;
    }
     ...
     */
    public final static boolean scaleByRatio(String srcImageFile, String targetImageFile, int scale, boolean flag) {
        boolean result = false;
        try {
            BufferedImage src = ImageIO.read(getImageFile(srcImageFile)); // 读入文件
            ...
            e.printStackTrace();
            logger.error("ImageUtil scaleByRatio from {} to {} failed!", srcImageFile, targetImageFile);
        }
        return result;
    }
     ...
     */
    public final static boolean scaleBySize(String srcImageFile, String targetImageFile, int width, int height, boolean bb) {
        boolean result = false;
        try {
            if (width == 0 || height == 0) {
            ...
            e.printStackTrace();
            logger.error("ImageUtil scaleBySize from {} to {} failed!", srcImageFile, targetImageFile);
        }
        return result;
    }
     ...
     */
    public final static boolean cut(String srcImageFile, String targetImageFile, Incise incise) {
        boolean result = false;
        try {
            // 读取源图像
            BufferedImage bi = ImageIO.read(getImageFile(srcImageFile));
            ...
            if (srcWidth > 0 && srcHeight > 0) {
                ...
                Graphics g = tag.getGraphics();
                g.drawImage(img, 0, 0, incise.getWidth(), incise.getHeight(), null); // 绘制切割后的图
                g.dispose();
                // 输出为文件
    ...
    public final static boolean pressText( String srcImageFile, String destImageFile, Text text, MaskPosi posi) {
        ...
        try {
            ...
            g.dispose();
            ImageIO.write(image, ImageType.JPG.getName(), new File(destImageFile));// 输出到文件流
            result = true;
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     */
    public final static boolean pressImage(String maskImg, String srcImageFile,String destImageFile,
                                           MaskPosi posi) {
        boolean result = false;
        try {
            Image src = ImageIO.read(getImageFile(srcImageFile));
    ...
    private static File getImageFile(String srcImageFile) throws FileNotFoundException{
        ...
        if (!f.exists()) throw new FileNotFoundException("srcImageFile:" + srcImageFile + "not fund exception!");
        f.canRead();
        f.canWrite();
        return f;
    }
